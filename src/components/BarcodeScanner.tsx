...
export function BarcodeScanner({ onScan, onClose }: BarcodeScannerProps) {
  ...
  const [isLiveScanning, setIsLiveScanning] = useState(true); // Nuovo stato per distinguere tra live e file
  
  const readerRef = useRef<BrowserMultiFormatReader | null>(null);
  const controlsRef = useRef<any>(null);

  // Funzione per interrompere lo streaming video
  ...
  const handleImageUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    // Se stiamo scansionando live, interrompiamo prima lo streaming
    if (isLiveScanning) {
        stopLiveScanner();
    ...
    try {
        const barcodeText = await readerRef.current.decodeFromImage(file);
        
        // Se la decodifica ha successo
        if (barcodeText) {
  ...
  const startLiveScanner = async () => {
    ...
    const decode = async (deviceId: string | null = null, constraints: MediaStreamConstraints | undefined = undefined) => {
        stopLiveScanner(); // Assicurati che lo scanner sia fermo
    ...
    try {
        // Tentativo 1: Fotocamera Posteriore (environment)
        ...
        // Tentativo 2: Fallback (cerca 'back'/'rear' o usa il primo dispositivo)
        try {
            ...
            } else if (fallbackErr.name === 'NotFoundError' || err.name === 'NotFoundError') {
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col">
      ...
      <div className="flex-1 relative flex items-center justify-center">
        {error ? (
          ...
          <>
            ...
            {/* Messaggio se non siamo in scansione live (es. dopo aver caricato un file) */}
            {!isLiveScanning && (
                <div className="text-white text-center px-4">
      ...
      <div className="bg-gray-900 px-4 py-4 text-center text-white flex justify-around items-center">
        ...
        {/* Bottone per riattivare la Scansione Live (solo se non è già attiva) */}
